...
RUN_ROOT = Path(os.environ.get('RUN_ROOT', PERSIST_ROOT / '5h_hyperparamtuning'))
if not str(RUN_ROOT).startswith(str(PERSIST_ROOT)):
    print(f'WARNING: RUN_ROOT={RUN_ROOT} is not on persistent storage; forcing to {PERSIST_ROOT}/5h_hyperparamtuning')
    RUN_ROOT = PERSIST_ROOT / '5h_hyperparamtuning'
RUN_ROOT.mkdir(parents=True, exist_ok=True)

DATA_PATH = Path(os.environ.get('DATA_PATH', RUN_ROOT / 'master_df.parquet'))
...
feature_sets = {
    ...
    'meta_err': feature_columns + meta_regression_err_columns + meta_classification_err_columns,
    
    'meta_sentiment': feature_columns + meta_regression_columns + meta_classification_columns + sentinemt_columns,
    'meta_emotion': feature_columns + meta_regression_columns + emotion_columns,
    ...
    'meta_all_nlp': feature_columns + meta_regression_columns + meta_classification_columns + sentinemt_columns + emotion_columns + unified_emotion_columns + stance_columns + finbert_columns,
    'meta_sector': feature_columns + meta_regression_columns + meta_classification_columns + sector_columns,
    'meta_sector_sentiment': feature_columns + meta_regression_columns + meta_classification_columns + sector_columns + sentinemt_columns,
    ...
    'meta_sector_all_nlp': feature_columns + meta_regression_columns + meta_classification_columns + sector_columns + sentinemt_columns + emotion_columns + unified_emotion_columns + stance_columns + finbert_columns,
    
    'meta_err_sentiment': feature_columns + meta_regression_columns + meta_classification_columns + meta_regression_err_columns + meta_classification_err_columns + sentinemt_columns,
    ...
    'meta_err_all_nlp': feature_columns + meta_regression_columns + meta_classification_columns + meta_regression_err_columns + meta_classification_err_columns + sentinemt_columns + emotion_columns + unified_emotion_columns + stance_columns + finbert_columns, 
    ...
    'meta_err_sector_sentiment': feature_columns + meta_regression_columns + meta_classification_columns + meta_regression_err_columns + meta_classification_err_columns + sector_columns + sentinemt_columns,
    ...
    'meta_err_sector_all_nlp': feature_columns + meta_regression_columns + meta_classification_columns + meta_regression_err_columns + meta_classification_err_columns + sector_columns + sentinemt_columns + emotion_columns + unified_emotion_columns + stance_columns + finbert_columns,    
...
feature_groups = {
    'base': ['meta', 'meta_err'],
    'nlp': ['meta_sentiment', 'meta_emotion', 'meta_unified_emotion', 'meta_finbert', 'meta_all_nlp', 'meta_err_sentiment', 'meta_err_emotion', 'meta_err_unified_emotion', 'meta_err_finbert', 'meta_err_all_nlp'],
    'sector': ['meta_sector', 'meta_sector_sentiment', 'meta_sector_emotion', 'meta_sector_unified_emotion', 'meta_sector_finbert', 'meta_sector_all_nlp', 'meta_err_sector', 'meta_err_sector_sentiment', 'meta_err_sector_emotion', 'meta_err_sector_unified_emotion', 'meta_err_sector_finbert', 'meta_err_sector_all_nlp'],
}
    ...
    for feature_group, group_sets in feature_groups.items():
        ...
        def objective(trial, problem_type=problem_type, group_sets=group_sets):
            params = {
                ...
                'horizon_steps': trial.suggest_categorical('horizon_steps', [5]),
        ...
        out_path = out_dir / f'optuna_tuning_{feature_group}_5H.csv'
