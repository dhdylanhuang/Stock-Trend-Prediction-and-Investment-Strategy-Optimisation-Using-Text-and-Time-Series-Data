...
CACHE_MAX_ITEMS = 512  # adjust if memory is tight
...
RUN_ROOT = Path(os.environ.get('RUN_ROOT', PERSIST_ROOT / 'benchmarking'))
if not str(RUN_ROOT).startswith(str(PERSIST_ROOT)):
    print(f'WARNING: RUN_ROOT={RUN_ROOT} is not on persistent storage; forcing to {PERSIST_ROOT}/benchmarking')
    RUN_ROOT = PERSIST_ROOT / 'benchmarking'
RUN_ROOT.mkdir(parents=True, exist_ok=True)

DATA_PATH = Path(os.environ.get('DATA_PATH', RUN_ROOT / 'ta_nlp_sector.parquet'))
RESULTS_ROOT = Path(os.environ.get('RESULTS_ROOT', RUN_ROOT / 'results-notemp'))
...
# DataLoader tuning for 2 CPU cores
NUM_WORKERS_TRAIN = 2
NUM_WORKERS_EVAL = 2
PIN_MEMORY = True
...
def _default_confusion_save_path(title):
    out_dir = globals().get("RESULTS_OUT_DIR")
    model = globals().get("model_type", "model")
    horizon = globals().get("horizon_tag", "1H")
    if out_dir is None:
        return None

    if "Row-normalized" in title:
        suffix = "agg_confusion_row_normalized.png"
    else:
        suffix = "agg_confusion_counts.png"
    return Path(out_dir) / f"{model}_{horizon}_{suffix}"

def plot_confusion(cm, labels, title="Confusion Matrix", save_path=None, show=False):
    ...
    if save_path is None:
        save_path = _default_confusion_save_path(title)
...
def _macro_f1_torch(y_hat_batch, y_true_oh, K, device):
    """
    Vectorised macro-F1 for a batch of prediction vectors.
    y_hat_batch : [B, N] int64 predicted class labels
    y_true_oh   : [N, K] float32 one-hot ground truth (pre-computed once)
    returns     : [B] float32 macro-F1, matching sklearn zero_division=0 behaviour
    """
    cls_range = torch.arange(K, device=device)
    pred_oh   = (y_hat_batch.unsqueeze(-1) == cls_range)   # [B, N, K]
    true_oh_b = y_true_oh.unsqueeze(0).bool()              # [1, N, K]
    tp = (pred_oh &  true_oh_b).float().sum(dim=1)         # [B, K]
    fp = (pred_oh & ~true_oh_b).float().sum(dim=1)
    fn = (~pred_oh &  true_oh_b).float().sum(dim=1)
    prec = tp / (tp + fp + 1e-8)
    rec  = tp / (tp + fn + 1e-8)
    f1   = 2.0 * prec * rec / (prec + rec + 1e-8)
    # only average over classes that actually appear in y_true (mirrors zero_division=0)
    present = y_true_oh.any(dim=0)                         # [K]
    f1 = f1 * present.float()
    return f1.sum(dim=1) / present.float().sum().clamp(min=1.0)  # [B]
...
def tune_temperature_and_taus(
    ...
    """
    Learns a single temperature T and per-head taus (K-1) by maximising
    macro-F1 minus a balance penalty.

    GPU acceleration: the innermost tau sweep is fully vectorised using
    tensor broadcasting — all tau candidates for a given coordinate are
    evaluated in one batched GPU op, replacing the Python loop +
    sklearn f1_score calls.  torch.cummin replaces the numpy monotone
    repair.  Falls back to CPU tensor ops on machines without CUDA/MPS.
    """
    device = (
    ...
    # Ground-truth one-hot — computed once, reused across all iterations
    ...
    y_oh.scatter_(1, y_t.unsqueeze(1), 1.0)                          # [N, K]
    ...
    def _score_batch(y_hat_batch):
        # y_hat_batch: [B, N] int
        f1s   = _macro_f1_torch(y_hat_batch, y_oh, K, device)                # [B]
        p_hat = (y_hat_batch.unsqueeze(-1) == cls_range).float().mean(dim=1)  # [B, K]
        pen   = (p_hat - p_target).abs().sum(dim=-1)                          # [B]
        return f1s - lambda_balance * pen                                      # [B]
    ...
    for T in T_grid_t:
        # sigmoid + monotone repair via cumulative min (equiv. to monotone_repair_numpy)
        P_rep, _ = torch.cummin(torch.sigmoid(Z_val_t / T), dim=1)   # [N, K-1]

        taus = torch.full((K_1,), 0.5, dtype=torch.float32, device=device)

        for _ in range(coord_rounds):
            for k in range(K_1):
                # Build [n_tau, K-1]: all tau candidates varying only at coordinate k
                taus_batch       = taus.unsqueeze(0).expand(n_tau, -1).clone()  # [n_tau, K-1]
                taus_batch[:, k] = tau_grid_t

                # Decode the full validation set for every tau candidate at once
                # [n_tau, N, K-1] comparison → [n_tau, N] class labels
                comp      = (P_rep.unsqueeze(0) >= taus_batch.unsqueeze(1)).int()
                y_hat_b   = comp.sum(dim=-1)                                     # [n_tau, N]

                taus[k]   = tau_grid_t[_score_batch(y_hat_b).argmax()]

        # Final score for this temperature after coordinate ascent
        y_hat_f = (P_rep >= taus).int().sum(dim=-1).unsqueeze(0)               # [1, N]
...
class StockPredictionPipeline:
    ...
    def process_company(self, company_name, company_data, sector):
        ...
        try:
            ...
            # No calibration in this notebook: keep temperature at 1.0 and fixed taus.
            T_star = 1.0
            taus = np.full(K - 1, 0.5, dtype=np.float32)

            Z_test = collect_logits(model, test_loader, self.device)
            P_cum  = 1.0 / (1.0 + np.exp(-Z_test.detach().cpu().numpy()))
...
for OPTUNA_CSV in optuna_csvs:
    ...
    for model_type in model_types:
        print(f"\n{'-'*20} Processing model type: {model_type} {'-'*20}")
        ...
        ordinal_head = row["params_ordinal_head"] if "params_ordinal_head" in row else "corn"
        ...
        batch_size = _get_param(row, 'params_batch_size', 64, int)
        ...
        if results_df is not None and not results_df.empty:
            ...
            if pipeline.problem_type in ['classification', 'multiclass']:
                ...
                if agg_cm is not None:
                    cm_dir = RESULTS_ROOT / 'multiclass' / feature_group
                    cm_dir.mkdir(parents=True, exist_ok=True)
                    counts_path = cm_dir / f"{model_type}_{horizon_tag}_agg_confusion_counts.png"
                    row_norm_path = cm_dir / f"{model_type}_{horizon_tag}_agg_confusion_row_normalized.png"
                    plot_confusion(
                        agg_cm,
                        display_labels,
                        title="Aggregated Confusion (Quantile buckets)",
                        save_path=counts_path
                    )
                    agg_norm = agg_cm / agg_cm.sum(axis=1, keepdims=True).clip(min=1)
                    plot_confusion(
                        agg_norm,
                        display_labels,
                        title="Aggregated Confusion (Row-normalized)",
                        save_path=row_norm_path
                    )
        else:
...
#             'horizon_steps': horizon_steps,
#             'n_classes': trial.suggest_categorical('n_classes', [4, 6]),
