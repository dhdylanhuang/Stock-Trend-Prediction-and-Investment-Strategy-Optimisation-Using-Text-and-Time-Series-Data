...
MODEL_SAVE_PATH = MODEL_SAVE_PATH if 'MODEL_SAVE_PATH' in globals() else Path('trained_models/')
...
# Select best H config from benchmarking and align feature set
from pathlib import Path
...
sector_columns = [
    ...
    'sector_range', 'sector_vol_20d', 'market_close', 'sector_rel_strength', 'sector_dispersion_1d',
]
...
def _resolve_benchmark_dir():
    ...
    ta_dir = root / 'classification-ta-nlp-sector'
...
def select_and_apply_best_config(horizon=5, model='ALL', metric='mcc'):
    best_cfg = select_best_config(model=model, horizon=horizon, metric=metric, base_dir=Path(os.environ.get('BENCHMARK_RESULTS_ROOT', '/mnt/primary/benchmarking/results')) / 'classification')
...
output_dir = OUTPUT_ROOT / 'simulation' / 'binary' / 'no-meta'
